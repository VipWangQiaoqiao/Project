...
public class LikeContainer extends LinearLayout {
	...
	AvatarView likeUser = (AvatarView) lInflater.inflate(
		R.layout.list_cell_like_user, null, false);
	likeUser.setAvatarUrl(user.getPortrait());
	int w = (int) this.context.getResources()
		.getDimension(R.dimen.space_25);
	...
	LayoutInflater lInflater = LayoutInflater.from(context);
	TextView more = (TextView) lInflater.inflate(
		R.layout.list_cell_like_more, null, false);
	int w = (int) this.context.getResources()
		.getDimension(R.dimen.space_26);
	LayoutParams params = new LayoutParams(w, w);
	params.setMargins(5, 0, 5, 0);
	...
	if (tweet.getIsLike() == 1) {
	    ...
	    tweet.getLikeUser().add(AppContext.getInstance().getLoginUser());
	}
    }
